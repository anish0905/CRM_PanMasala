...
export const ExecutiveReports = () => {
  ...
  const handleViewButtonClick = (filedManager) => {
    nagivate(`/Vendor-details/${filedManager._id}`);
  };
                  ...
                  {paginatedFiledManagers().map((filedManager) => (
                    <tr key={filedManager._id} className="bg-white rounded-lg">
                      ...
                      <td className="px-2 py-4 md:text-lg text-xs text-center">
                        {filedManager?.inspectionShops.length}
                      </td>
                      <td className="px-2 py-4 md:text-lg text-xs text-left flex gap-4">
                        <button
                          onClick={() => handleViewButtonClick(filedManager)}
