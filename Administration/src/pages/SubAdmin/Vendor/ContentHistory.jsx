  ...
  const fetchHistoty = async () => {
    try {
      const resp = await axios.get(
        `${BASE_URL}/api/inspectionShop/history/${shop._id}`
      );
      ...
      <div className="flex-1  lg:ml-80  font-serif w-full lg:p-10 md:p-5 ">
        <header className="flex  items-center justify-between h-auto py-4 bg-[#93c5fd] rounded-xl px-4 lg:px-10 md:px-8">
          ...
          <div className="flex items-center gap-2">
            <button
              className="text-xs sm:text-sm lg:text-lg font-bold text-white border-2 sm:border-4 border-blue-900 px-3 sm:px-4 py-1 sm:py-2 rounded-lg bg-blue-700 hover:bg-blue-800 transition duration-300 ease-in-out cursor-pointer"
              onClick={() => navigate(-1)}
            >
              Back
            </button>
        ...
        <section className="bg-gray-100 rounded-lg shadow-lg p-5">
          ...
          {history && history.length > 0 ? (
            <div className="space-y-6">
              {history.map((entry) => {
                const { field, oldValue, newValue, timestamp, updatedByUser } =
                  entry;
