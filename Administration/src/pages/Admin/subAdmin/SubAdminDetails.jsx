...
const SubAdminDetails = () => {
  ...
  const fetchsubAdmins = async () => {
    try {
      const response =
        location.pathname === "/manage/Sub-Admin/Registration/Sub-Admin"
  ...
  return (
    <div className="flex gap-6 bg-[#dbeafe] min-h-sreen w-full">
     { role ==="Sub-Admin" && ( <div className="h-screen md:block lg:block hidden">
        <AdminSidebar />
      </div>)}
       {role === "supersubAdmin" && (
      ...
      <div className="lg:ml-80 md:ml-40 font-serif w-full  md:p-5 p-4">
        <div className="bg-[#93c5fd] rounded-md shadow p-4 flex gap-4 items-center justify-between">
          ...
          { role ==="Sub-Admin" && (<div className="lg:hidden md:hidden block">
            <AdminSideBarModal />
          </div>)}
        </div>
